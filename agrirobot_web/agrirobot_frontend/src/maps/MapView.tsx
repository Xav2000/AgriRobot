    ...
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      <MapContainer center={center} zoom={15} style={{ width: '100%', height: '100%' }}>
        ...
        {mapData.zones.map((zone) => (
          <LeafletPolygon key={zone.id} positions={zone.points.map(p => [p.lat, p.lng])} color={zone.color || '#4CAF50'} fillOpacity={0.5} weight={2} eventHandlers={{ click: () => setSelectedZone(zone) }} />
        ))}
        {mapData.worklines.map((workline) => (
          <Polyline key={workline.id} positions={workline.points.map(p => [p.lat, p.lng])} color="#FF5733" weight={2} />
        ))}
        {mapData.paths.map((path) => (
          <Polyline key={path.id} positions={path.points.map(p => [p.lat, p.lng])} color="#2196F3" weight={3} dashArray="5, 5" />
        ))}
        <ZoneEditor mapData={mapData} setMapData={setMapData} />
      </MapContainer>
      <div style={{ position: 'absolute', top: '20px', right: '20px', zIndex: 1000, background: 'white', padding: '16px', borderRadius: '8px', boxShadow: '0 2px 10px rgba(0,0,0,0.2)', minWidth: '280px' }}>
        ...
        <div style={{ marginTop: '16px', textAlign: 'center' }}>
          <button onClick={() => { setMapData(initialMapData); setSelectedZone(null); }} style={{ padding: '8px 16px', background: '#f44336', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>
            Tout effacer
          </button>
        </div>
      </div>
